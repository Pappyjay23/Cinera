...
interface GenreFilterProps {
	...
	mediaType: "all" | "movie" | "tv" | undefined;
}
...
const GenreFilter = ({
	...
	const { data: genresData } = useQuery({
		queryKey: ["genres", "list", mediaType],
		queryFn: () => movieService.getGenresList(mediaType),
		staleTime: Infinity,
	});
	...
	const genres = (genresData || []).map((g: Genre) => g.name).length
		? (genresData || []).map((g: Genre) => g.name)
		: fallback;
